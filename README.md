...
A client-side [Fabric](https://fabricmc.net/) mod for **Minecraft 26.2** that records what you do —
movement, key actions, look direction, chat and commands — and replays it on demand, saved by name.

Mod id: `macromod` · client-only (`"environment": "client"`) · MIT

## Commands

| Command | Behaviour |
| --- | --- |
| `/macro record` | Start capturing. Errors if already recording or playing. |
| `/macro stop` | Stop recording — or cancel a running playback. Keeps the take in memory as unsaved. |
| `/macro save <name>` | Write the last take to `config/macromod/<name>.json`. While recording, implies stop-then-save. |
| `/macro play <name> [--no-look]` | Replay a macro. `--no-look` ignores the recorded look direction. Tab-completes saved names. |
| `/macro list` | List saved macros with tick length, duration and event count. |
| `/macro delete <name>` | Delete a macro. Tab-completes. |

Names are restricted to `[a-zA-Z0-9_-]{1,32}` so they are always path-safe.
...
## Read this before touching the code: 26.x is unobfuscated
...
When in doubt about an API, read the real jar rather than trusting a tutorial written for 1.21:
...
1. **Discrete taps come from the mixin, not from polling.** A key pressed and released between two
   ...
   this way during testing. `KeyMappingMixin` hooks `KeyMapping.set`, which both the keyboard and
...
Useful trick: **F2 takes a screenshot into `run/screenshots/`**, which is an easy way to capture
in-game state (selected hotbar slot, chat history, facing) for a before/after comparison.

## Known limits

- This replays *input*, not *state*. Long macros drift if your position, the world, or server lag
  differ from recording time.
- Don't touch the keyboard during playback — real key events write the same `KeyMapping` state the
  macro drives, and they will fight.
- Toggle-sneak and toggle-sprint change how vanilla reads those keys; macros recorded with one setting
  may behave differently with the other.
